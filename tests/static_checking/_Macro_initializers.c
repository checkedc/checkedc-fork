...
extern void f1() {
  int* _Array v1;
  int* _Array v2 = 0;
  int* _Array v3  _Bounds(unknown);
  int* _Array v4  _Bounds(unknown) = 0;
  int* _Array v5  _Count(5) = 0;
  int* _Array v6  _Count(5);                       // expected-error {{automatic variable 'v6' with bounds must have initializer}}
  int* _Array v7  _Byte_count(5 * sizeof(int)) = 0;
  int* _Array v8  _Byte_count(5 * sizeof(int));    // expected-error {{automatic variable 'v8' with bounds must have initializer}}
  int* _Array v9  _Bounds(v9, v9 + 5) = 0;
...
int* _Nt_array g30  _Count(4) = (int[]) { 0, [2] = 2, 3, 5, 0 };
int* _Nt_array g31  _Count(0) = (int checked[]) { 0, [2] = 2, 3, 5, 0 };
char* _Nt_array g32  _Count(5) = "abcde";
char* _Array g33  _Count(5) = "abcde";
char* _Array g34  _Count(5) = (char[5]) { 'a', 'b', 'c', 'd', 'e' };
char* _Array g35  _Count(5) = (char checked[5]) { 'a', 'b', 'c', 'd', 'e' };

//
// Checked arrays of checked pointers
//
char* _Nt_array g36 checked[3][2] = { [1] = "ab", "cd", "ef", "jk" };
char* _Nt_array g37 nt_checked[] = { "the", "brown", "fox", "jumped",
                                        "over", "the", "fence", 0 };
...
void f4(void) checked {
  ...
  char* _Nt_array t23 = g23[0]; // expected-warning {{cannot prove declared bounds for 't23' are valid after initialization}}
  int t30 = g30[1];
  int t31 = *g31;
  char t32 = g32[5];
  char t33 = g33[4];
  char t34 = g34[4];
  char t35 = g35[1];
  char* _Nt_array t36 = g36[1][0];
  char* _Nt_array t37 = g37[1];
  ...
  int* _Nt_array t100  _Count(3) = (int[]) { 0, 1, 2, 3 };
}
...
void f5(void) checked {
  ...
  int* _Nt_array t30  _Count(4) = (int[]) { 0, [2] = 2, 3, 5, 0 };

  int* _Nt_array t31 = (int checked[]) { 0, [2] = 2, 3, 5, 0 };
  char* _Nt_array t32  _Count(5) = "abcde";

  char* _Array t33  _Count(5) = "abcde";
  ...
  //
  int* _Nt_array t36  _Count(4) = ((int[]) { 0, [2] = 2, 3, 5, 0 });
  char* _Nt_array t37  _Count(5) = ("abcde");

  //
  // Checked arrays of checked pointers.
  //
  char* _Nt_array t38 checked[3][2] = { [1] = "ab", "cd", "ef", "jk" };
...
void f6(void) {
  ...
  int* _Array q  _Count(5) = 0;  // has bounds; initializer required.
 ...
 // Check { 0 } initialization idiom where first member is a floating point number.
  ...
  struct checked_value_has_bounds {
    int x;
    char* _Array lower  _Count(5);
    char* _Array upper  _Count(5);
    float y;
  };
...
void f7() {
  ...
  int x  _Count(5); // expected-error {{with a bounds expression must have an initializer}} expected-error {{have a pointer or array type}}
  int y  _Count(6) = 0; // expected-error {{have a pointer or array type}}
}
...
void f8 (void) {
  ...
  typedef struct {
    int a;
    unsigned long long b  _Bounds( (int* _Array) b, (int* _Array) b + 10);
  } S0;
  ...
  // for bounds expr kind like "int i  _Count(len)"
  typedef struct {
    int a;
    int b  _Count(10);  // expected-error {{have a pointer or array type}}
  } S;
...
void f9 (void) checked {
  int a;
  float b;
  int* p  _Count(1) = &a; // expected-error {{bounds declaration not allowed for local variable with unchecked pointer type}}
  char* s  _Count(10); // expected-error {{bounds declaration not allowed for local variable with unchecked pointer type}}
}

// test unchecked pointer with bounds expression, in a checked scope
void f10 (void) checked {
  char* p  _Count(5); // expected-error {{bounds declaration not allowed for local variable with unchecked pointer type}}
}

// For unchecked_pointer_with_bounds_expr_in_checked_scope, we need to consider struct also
void f11 (void) checked {
  int a;
  int* p  _Count(5); // expected-error {{bounds declaration not allowed for local variable with unchecked pointer type}}
  void* src  _Count(10);  // expected-error {{bounds declaration not allowed for local variable with unchecked pointer type}}
