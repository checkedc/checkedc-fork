...
checked int f2(int *s  _Count(len), int len) {
...
checked int f5(int *s  _Itype(int checked[])) {
  int *_Array t1 = s + 4;
  int t2 = *s;                // expected-error {{expression has unknown bounds}}
  int t3 = s[4];              // expected-error {{expression has unknown bounds}}
  *(s + 4) = 0;               // expected-error {{expression has unknown bounds}}
...
int g4[5]  _Itype(int checked[5]);
extern int g5[]  _Itype(int checked[]);
...
//
void f10(int *p  _Itype(int *_Single));
void f11(int *p  _Count(4));
void f12(int *p  _Itype(int *_Array));
void f13(int *p  _Itype(int checked[]));
void f14(int *p  _Itype(int checked[4]));
void f15(int *_Single p, int *q  _Itype(int *_Single));


extern int empty_global_arr[]  _Itype(int checked[]);

// Check calls to those functions.
checked int test_call_parameters(void) {
  int *_Single param1 = 0;
  int *_Array param2  _Count(4) = 0;
  int *_Array param3;
  int arr1 checked[4];
  f10(param1);
  f10(param2);
...
checked void f11e(char *buf : bounds(buf, end),
                  char *end  _Itype(_Array_ptr<char>));
checked void f12a(int *p  _Itype(int *_Array));
checked void f13a(int *p  _Itype(int checked[]));
checked void f14a(int *p  _Itype(int checked[4]));
checked void f15a(int *_Single p, int *q  _Itype(int *_Single));
...
checked int *f21a(int *b  _Count(4))  _Count(4) {
  return b;
}
...
checked int *f21c(int *b  _Count(4)) : bounds(return_value, return_value + 4) {
  return b;
}

checked int *f21d(int *b  _Count(4)) :
...
checked int* f22()  _Itype(int *_Array);  // expected-error {{function without a prototype cannot be used or declared in a checked scope}}
...
checked int test_struct1(struct S1 *p  _Itype(ptr<struct S1>)) {
...
int test_struct2(struct S1 *p  _Itype(ptr<struct S1>)) {
  int t1 = *(p->f1 + 4);
  int t2 = *(p->f2 + 4);
  int t3 = *(p->f3 + 4);
  int t4 = *(p->f4 + 4);
...
checked int f50(int **s  _Itype(ptr<int *_Single>)) {
  ...
  *s += 5;            // expected-error {{arithmetic on _Ptr type}}
  ...
  int *_Array t5  _Count(1) = *s;
  *s = t5;

  float* _Array t6  _Count(1) = *s; // expected-error {{incompatible type}}
  float* _Array t7 = 0;
...
typedef ptr<int(int *a  _Count(n), int n)> callback_fn1;
...
typedef int (*callback_fn3)(int *a  _Count(n), int n);
// Corresponding version with bounds-safe interface.
typedef  ptr<int (int *a  _Count(n), int n)> bsi_callback_fn3;

checked callback_fn3 return_function_pointer(void)  _Itype(bsi_callback_fn3);
...
struct S2 {
  ...
  int *f3  _Count(len);
  int len;
  int *f4  _Itype(int *_Array);
  int arr[5]  _Itype(int checked[5]);
  // pointer to function that returns a ptr<int *_Single>
...
checked int test_struct3(struct S2 *p  _Itype(ptr<struct S2>)) {
  ...
  *((*(p->fp1))(p->f1)) + 5; // expected-error {{arithmetic on _Ptr type}}
  ...
  // should return a _int *_Single.  Then dereference the _int *_Single
  int i = *(*(p->fp2))(p->f1, 5)[4];
  // Subscript the result returned by the indirect function call by 4.  This
  // should return a _int *_Single.  Then try to do pointter arithmetic on the resulting
  // pointer.
...
unchecked int test_struct4(struct S2 *p  _Itype(ptr<struct S2>)) {
  ...
  *((*(p->fp1))(p->f1)) + 5;
  ...
  int i = *(*(p->fp2))(p->f1, 5)[4];
  // Subscript the result returned by the indirect function call by 4.  This
  // should return a _int *_Single.  Then do pionter arithmetic on the int *
...
base_with_interfaces table2[10]  _Itype(fnptr_interface checked[10]);
...
checked void test1_array_of_function_pointers(int *_Single arg1, ptr<int *_Single> arg2, int num) {
  int *_Single result1 = (*(table1[num]))(arg1, arg2);
  ...
  int *_Single result2 = (*(table2[num]))(arg1, arg2);
}
...
unchecked void test2_array_of_function_pointers(int *_Single arg1, ptr<int *_Single> arg2, int num) {
  (*(table1[num]))(arg1, arg2); // expected-error {{passing '_Ptr<int>' to parameter of incompatible type 'int *'}}
  int *_Single result1 = (*(table1[num]))(0, 0) + 5; // expected-error {{expression has unknown bounds}}
  (*(table1[num]))(0, 0) + 5;
  (*(table2[num]))(arg1, arg2);
  (*(table2[num]))(arg1, arg2) + 5;
  int *_Single result = (*(table2[num]))(arg1, arg2);
}
...
int mblen(const char *s  _Count(n), size_t n);
