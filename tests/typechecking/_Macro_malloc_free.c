...
void f12(void) unchecked {
    ...
    int* _Single x = calloc(1, sizeof(int));
    free(x);
}
...
void f13(void) unchecked {
    // Intentionally vary test from f11: type argument can be omitted
    // in an unchecked scope.
    int* _Single x = malloc(sizeof(int));
    int* _Single y = realloc _TyArgs(int) (x, 2 * sizeof(int));
    free(y);
}

// Test you can always `free` a `aligned_alloc`d ptr
void f14(void) {
    int* _Single x = aligned_alloc(_Alignof(int), sizeof(int));
    free _TyArgs(int) (x);
}
...
void f22(void) {
    int* _Array x  _Count(4) = calloc _TyArgs(int) (4, sizeof(int));
    free _TyArgs(int) (x);
}
...
void f24(void) {
    int* _Array x  _Count(4) = aligned_alloc(_Alignof(int), 4 * sizeof(int));
    free _TyArgs(int) (x);
}
