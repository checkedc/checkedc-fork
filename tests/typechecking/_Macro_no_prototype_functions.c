...
extern int* _Array f2();// expected-error {{function with no prototype cannot have a return type that is a checked type}}
...
// Returns an unchecked pointer to a function with a checked argument (int* _Single)
extern int (*f19())(int* _Single);  // expected-error {{function with no prototype cannot have a return type that is a checked type}}
...
// Returns a checked pointer to a function with a checked argument (int* _Single)
extern ptr<int (int* _Single)> f21(); // expected-error {{function with no prototype cannot have a return type that is a checked type}}

// Returns a checked pointer to a no-prototype function returning a checked
// value (int* _Single)
// This unexpectedly produces a syntax error
// extern ptr<int* _Single ()> f21();
...
extern int* _Array f50() : count(5); // expected-error {{function with no prototype cannot have a return type that is a checked type}}
extern int f51() : byte_count(10);
extern int *f52() : byte_count(10);
extern int *f53() : itype(int* _Single);
...
extern void f60(); 
extern void f60(int* _Single); // expected-error {{cannot redeclare a function with no prototype to have an argument type that is a checked type}}
...
extern void f82();
extern void f82(int f(int* _Single)); // expected-error {{cannot redeclare a function with no prototype to have an argument type that is a checked type}}

extern void f83(int f());
extern void f83(int f(int* _Single));  // expected-error {{conflicting types for 'f83'}}

extern void f84(int f(int* _Single));  
...
typedef int functype2(int* _Single);
struct S22 {
  functype2 *m;
};
