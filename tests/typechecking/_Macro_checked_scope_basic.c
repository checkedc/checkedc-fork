...
void checked_local_types(void) SCOPE_KIND {
  ...
  int* _Single *t11 = 0;       // expected-error {{local variable in a checked scope must have a checked type}}
  int* _Array *t12 = 0; // expected-error {{local variable in a checked scope must have a checked type}}
...
scope must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}}
  ...
  int* _Array t33[10] = 0;   // expected-error {{local variable in a checked scope must have a checked type}}
...
scope must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}}
  int* _Single t44 checked[10] = {0};
  int* _Array t45 checked[10] = {0};

  int *t46 checked[10][15];  // expected-error {{local variable in a checked \
scope must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}}
  int* _Single *t47 checked[10][15];  // expected-error {{local variable in a checked \
scope must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}}
...
scope must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}}
  ptr<void(int* _Single)> t64 = 0;
  ptr<void(int* _Array)> t65 = 0;
...
SCOPE_KIND int* func6(int a[][5], int b[][5]) : itype(int* _Single) SCOPE_KIND { // expected-error 2 {{parameter in a checked scope must have a checked type or a bounds-safe interface}}
}

SCOPE_KIND int* func7(void) : itype(int* _Single) SCOPE_KIND {
 return 0;
}
...
SCOPE_KIND int func9(int* _Single p) {
...
int* _Array func12(int* _Array x, int len) : bounds(x,x+len) SCOPE_KIND {
  return 0;
}

int* _Array func13(int *x, int *y) SCOPE_KIND {
...
int func26(int* _Single p, int *q) {
  int a = 5;
  int *upa;
  SCOPE_KIND {
...
SCOPE_KIND int func42(int* _Single p, int q[], int r[5]) unchecked {  // expected-error 2 {{parameter in a checked scope must have a checked type or a bounds-safe interface}}
  int a = 5;
  SCOPE_KIND {
    int *upa = &a;  // expected-error {{local variable in a checked scope must have a checked type}}
  unchecked {
...
int func46(int* _Single p, int *q) unchecked {
  int a = 5;
  int *upa;
  int* _Array pc;
  SCOPE_KIND {
...
unchecked int * func51(int *p, int* _Single q, int* _Array r, int* _Array s _Count(2)) {
  int a = 5;
  SCOPE_KIND {
...
unchecked int func56(int* _Single p, int *q) unchecked {
  int a = 5;
  int *upa;
  int* _Array pc;
  SCOPE_KIND {
...
unchecked int* _Array func57(void) {
  int *upa;
  unchecked {
  int *upb = 0;
  int* _Array pb;
...
// ex) checked { .... int a; int* _Single pb = &a; }
void test_addrof_checked_scope(void) SCOPE_KIND {
  ...
  int* _Single x = &a[i]; // expected-warning {{cannot prove cast source bounds are wide enough for '_Ptr<int>'}} \
                         ImplicitCastExpr _Ptr (UnaryOperator _Array_Ptr<int>)
  int* _Single y = &b[0]; // ImplicitCastExpr _Ptr (UnaryOperator _Array_Ptr<int>) \
                      // expected-error {{expression has unknown bounds, cast to ptr<T> expects source to have bounds}}
  int* _Single z = &i;    // ImplicitCastExpr _Ptr (UnaryOperator _Array_Ptr<int>)
  ...
  // &*(b+i) result type = type of (b+i) = int* _Array
  // &b[i] result type = int* _Array
  int* _Array px = &(*(b+i));
  int* _Array py = &b[i];
  int* _Array pz = &(*(a+i));
...
void test_addrof_unchecked_scope(void) unchecked {
  ...
  int* _Single x = &a[i]; // ImplicitCastExpr _Ptr<int>(UnaryOperator int * prefix &)
  int* _Single y = &b[0]; // ImplicitCastExpr _Ptr<int>(UnaryOperator int * prefix &) \
                      // expected-error {{expression has unknown bounds, cast to ptr<T> expects source to have bounds}}
  int* _Single z = &i;    // ImplicitCastExpr _Ptr<int>(UnaryOperator int * prefix &)
  ...
  int* _Array px = &(*(b+i));  // UnaryOperator _Array_ptr<int>(ParenExpr(UnaryOperator(*))
  int* _Array py = &b[i];      // ImplicitCastExpr _Array_ptr<int>(UnaryOperator)
  int* _Array pz = &(*(a+i));  // UnaryOperator _Array_ptr<int>(ParenExpr(UnaryOperator(*)))
...
int *gptr0 : itype(int* _Single);
int *gptr1 : itype(int* _Array);
int *gptr2;
...
extern void check_assign(int val, int p[10], int q[], int r checked[10], int s checked[],
  ...
  int (*_Array t22) [10], (*_Array t23)[10], (*_Array t24)[10];
  int (*_Array t25) checked[10], (*_Array t26) checked[10],
      (*_Array t27) checked[10];
...
SCOPE_KIND extern void f7(int (*_Array p)[10], int y) { // expected-error {{must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}} \
...
extern void f9(int (*_Single p )checked[10] , int y) {
}

extern void f10(int (*_Array p) checked[10], int y) {
}
...
SCOPE_KIND int* _Single h7(void) {
  return checked_global; // ptr<T> = array_ptr<T> OK
}

SCOPE_KIND int* _Array h8(void) {
...
SCOPE_KIND int* _Single h10(void) {
...
SCOPE_KIND int* _Single h13(int arr checked[10]) {
  return arr;  // ptr<T> = array_ptr<T> OK
}

SCOPE_KIND int* _Array h14(int arr checked[10]) {
  return arr;
}
...
SCOPE_KIND unchecked_arr_type *_Array h21  (int arr[10][10]){ // expected-error {{must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}} \
                                                  // expected-error {{parameter in a checked scope must have a checked type or a bounds-safe interface}}
  return arr;
}
...
SCOPE_KIND unchecked_arr_type* _Single h23(int arr checked[10][10]) { // expected-error {{must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}}
  return 0;
}

SCOPE_KIND unchecked_arr_type* _Array h24(int arr checked[10][10]) { // expected-error {{must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}}
  return 0;
}
...
typedef int checked_arr_type checked[10] ;
SCOPE_KIND checked_arr_type* _Single h26(int arr checked[10][10]) {
  return arr;  // ptr<T> = array_ptr<T> OK
}

SCOPE_KIND checked_arr_type* _Array h27(int arr checked[10][10]) {
  return arr;
}
...
SCOPE_KIND void check_cast_operator(void) {
  ...
  typedef int checked_arr_type_5 checked[5] ;
  typedef int unchecked_arr_type_5 [5] ;
  parr = (checked_arr_type_5* _Single) &arr;
  parr = (checked_arr_type_5* _Single) ((ptr<int checked[]>) &arr); // expected-warning {{cannot prove cast source bounds are wide enough for '_Ptr<checked_arr_type_5>' (aka '_Ptr<int _Checked[5]>')}}
  parr = (unchecked_arr_type_5* _Single) &arr;   // expected-error {{type in a checked scope must use only checked types or parameter/return types with bounds-safe interfaces}}
  parr = (int **_Single) &arr;     // expected-error {{type in a checked scope must use only checked types or parameter/return types with bounds-safe interfaces}}

  // array_ptr to array, array_ptr to unchecked array
  int (* _Array aparr) checked[5] = 0;
  aparr = (checked_arr_type_5* _Array) &arr;
  aparr = (checked_arr_type_5* _Array) ((array_ptr<int checked[]>) &arr);
  aparr = (unchecked_arr_type_5* _Array) &arr;  // expected-error {{type in a checked scope must use only checked types or parameter/return types with bounds-safe interfaces}}

  // ptr to variadic/unchecked func pointer
  int (* _Single vpa)(int,int)  = 0;
  vpa = (int (*_Single)(int,...)) &arr;  // expected-error {{type in a checked scope cannot have variable arguments}}
  vpa = (int (*_Single)(int,int (*_Single)(int, ...))) &arr;  // expected-error {{type in a checked scope cannot have variable arguments}}
  vpa = (int* (*_Single)(int,int)) &arr;  // expected-error {{type in a checked scope must use only checked types or parameter/return types with bounds-safe interfaces}}
  vpa = (int(*_Single)(int,int(*_Single)(int, ...), ...)) &arr;  // expected-error {{type in a checked scope cannot have variable arguments}}

  int *upa;                         // expected-error {{local variable in a checked scope must have a checked type}}
  upa = arr;
  upa = (int *)(int* _Array)arr; // expected-error {{type in a checked scope must be a checked pointer or array type}}
  upa = &x;
  ...
  upa = pax;
  upa = (int *)(int* _Array)pax; // expected-error {{type in a checked scope must be a checked pointer or array type}}

  upa = aparr;
  upa = (int *)aparr;               // expected-error {{type in a checked scope must be a checked pointer or array type}}

  gptr0 = upa;
  gptr0 = (int* _Single)upa;
  gptr0 = (int* _Array)upa;

  gptr1 = upa;
  gptr1 = (int* _Single)upa;
  gptr1 = (int* _Array)upa;

  gptr2 = upa;                      // expected-error {{variable used in a checked scope must have a checked type}}
  gptr2 = (int* _Single)upa;            // expected-error {{variable used in a checked scope must have a checked type}}
  gptr2 = (int* _Array)upa;      // expected-error {{variable used in a checked scope must have a checked type}}
  ...
  SCOPE_KIND {
  p = _Dynamic_bounds_cast_M_N(int *, q);           // expected-error {{local variable used in a checked scope must have a checked type}} \
                                                // expected-error {{type in a checked scope must be a checked pointer or array type}} \

  p = _Dynamic_bounds_cast_M_N(int *, r);           // expected-error {{local variable used in a checked scope must have a checked type}} \
                                                // expected-error {{type in a checked scope must be a checked pointer or array type}} \

  q = _Assume_bounds_cast_M_N(ptr<int>, r);             // expected-error {{_Assume_bounds_cast not allowed in a checked scope or function}}
  r = _Assume_bounds_cast_M(array_ptr<int> , r,count(4));     // expected-error {{_Assume_bounds_cast not allowed in a checked scope or function}}
  r = _Assume_bounds_cast_M(array_ptr<int> , r,bounds(r,r+1)); // expected-error {{_Assume_bounds_cast not allowed in a checked scope or function}}

  vpa = _Dynamic_bounds_cast_M_N(ptr<int(int,...)> ,r);   // expected-error {{type in a checked scope cannot have variable arguments}}
  vpa = _Dynamic_bounds_cast_M_N(ptr<int(int,ptr<int(int, ...)>)> ,r);  // expected-error {{type in a checked scope cannot have variable arguments}}
  vpa = _Dynamic_bounds_cast_M_N(ptr<int*(int,int)>,r);  // expected-error {{type in a checked scope must use only checked types or parameter/return types with bounds-safe interfaces}}
  vpa = _Dynamic_bounds_cast_M_N(ptr<int(int,ptr<int(int, ...)>, ...)>, r);  // expected-error {{type in a checked scope cannot have variable arguments}}

  vpa = _Assume_bounds_cast_M_N(ptr<int(int,...)> ,r);   // expected-error {{_Assume_bounds_cast not allowed in a checked scope or function}}
...
SCOPE_KIND void check_checked_constructed_type(void) {
  int**_Single a = 0;                                  // expected-error {{must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}}
  char**_Single*_Array b _Count(5) = 0;           // expected-error {{must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}}
  int** _Single * _Single * _Single * _Array c _Count(10) = 0; // expected-error {{must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}}
  int**_Single*_Single*_Single d = 0;                        // expected-error {{must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}}
  int sum = 0;
  ...
  int*_Single pa[10] = 0;              // expected-error {{local variable in a checked scope must have a checked type}}
  int (* _Single pb) checked[10] = 0;
  int (*_Single *_Single pc )[5] = 0;          // expected-error {{local variable in a checked scope must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}}
  int (*_Single *_Array pd) [5] = 0;    // expected-error {{must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}}
  int (*_Single *_Single pe) checked[5]= 0;
  int (*_Single *_Single *_Single pf) [] = 0;      // expected-error {{must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}}
  int (*_Single *_Single *_Single pg)  checked[] = 0;
}
...
SCOPE_KIND void checked_check_variadic1(int (*fptr)(int, int* _Single, int* _Array, ...)) { // expected-error {{parameter in a checked scope must have a checked type or a bounds-safe interface}} \
...
SCOPE_KIND void checked_check_variadic3(ptr<int(int, int* _Single, int* _Array, ...)> fptr) { // expected-error {{parameter in a checked scope cannot have variable arguments}}
  int* _Single a = 0;
  int* _Array b;
  (*fptr)(5, a, b, a, b, a);
}

void checked_check_variadic4(ptr<int(int, int* _Single, int* _Array, ...)> fptr) SCOPE_KIND {
  int* _Single a = 0;
  int* _Array b;
...
int* _Single id_checked_intp(int* _Single x) { return 0; }

extern void test_function_pointer(void) SCOPE_KIND {
  // address-of (&) operator produces ptr<T> for function type
  int (*_Single fn0)(int) = &id_int;
  int (*_Single fn1)(int) = id_int;
  ...
  int (*_Single fn4)(int*) = &id_intp;   // expected-error {{must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}}
  int (*_Single fn5)(int*) = id_intp;  // expected-error {{must have a pointer, array or function type that uses only checked types or parameter/return types with bounds-safe interfaces}}
...
extern void test_cast_to_nt_array_ptr(void) checked {
  ...
  int* _Single p0 = (int* _Single)&i;
  int *_Nt_array p1 = (int *_Nt_array)p0; // expected-error {{'_Ptr<int>' cannot be cast to '_Nt_array_ptr<int>' in a checked scope because '_Ptr<int>' might not point to a null-terminated array}} 
  char*_Single p2 = (char*_Single)&c;
  char* _Nt_array p3 = (char* _Nt_array)p2; // expected-error {{'_Ptr<char>' cannot be cast to '_Nt_array_ptr<char>' in a checked scope because '_Ptr<char>' might not point to a null-terminated array}} 
  char*_Single p4 = "hello";
  p2 = (char* _Nt_array)p4;  // expected-error {{'_Ptr<char>' cannot be cast to '_Nt_array_ptr<char>' in a checked scope because '_Ptr<char>' might not point to a null-terminated array}}

  // Test casting array_ptr to nt_array_ptr.
  int* _Array p5 _Count(5) = arr_i;
  int *_Nt_array p6 = (int *_Nt_array)arr_i; // expected-error {{'_Array_ptr<int>' cannot be cast to '_Nt_array_ptr<int>' in a checked scope because '_Array_ptr<int>' might not point to a null-terminated array}}
  p6 = (int *_Nt_array)p5; // expected-error {{'_Array_ptr<int>' cannot be cast to '_Nt_array_ptr<int>' in a checked scope because '_Array_ptr<int>' might not point to a null-terminated array}}
  array_ptr<char> p7 _Count(5) = arr_c;
  ...
  char* _Nt_array p9 = "hello";
  char* _Nt_array p10 = (char* _Nt_array)p9;  // legal casting

  // Test casting to nt_array_ptr in a unchecked scope.
  unchecked {
    // Test casting ptr to nt_array_ptr.
    p1 = (int *_Nt_array)p0;   // allowed here; disallowed in a checked scope.
    p3 = (char* _Nt_array)p2;  // allowed here; disallowed in a checked scope.
    p2 = (char* _Nt_array)p4;  // allowed here; disallowed in a checked scope.
  
    // Test casting array_ptr to nt_array_ptr.
    p6 = (int *_Nt_array)arr_i; // allowed here; disallowed in a checked scope.
    ...
    // Test casting nt_array_ptr to nt_array_ptr
    p10 = (char* _Nt_array)p9;  // allowed in both checked and unchecked scopes.
    
    // Test casting from unchecked pointers to nt_array_ptr.
    int arr_i_unchecked[5];
    p6 = (int *_Nt_array)arr_i_unchecked;  // allowed
