...
extern void f1() {
  ...
  a = _Dynamic_bounds_cast_M(array_ptr<int>, b, count(10));
...
extern void f2() {
  ...
  array_ptr<int> a  _Count(1) = 0;
  int b checked[10];
  array_ptr<int> c  _Count(10) = (array_ptr<int>)a; // expected-error {{declared bounds for 'c' are invalid after initialization}}
  array_ptr<int> d  _Count(10) = _Dynamic_bounds_cast_M(array_ptr<int> , a, count(5)); // expected-error {{declared bounds for 'd' are invalid after initialization}}
  a = _Assume_bounds_cast_M_N(array_ptr<int>, p); // expected-error {{expected _Ptr or * type}}
}

extern void f3() {
  char p[10];
  ptr<int>c = 0;
  array_ptr<int> a  _Count(2) = 0;
  a = _Assume_bounds_cast_M (array_ptr<int> , p, count(p + 2)); // expected-error {{invalid argument type 'char *' to count expression}}
  c = _Assume_bounds_cast_M(ptr<int>,p, count(1)); // expected-error {{expected _Array_ptr type}}
}
...
extern void f4() {
  ...
  a = _Assume_bounds_cast_M_N (array_ptr<int> ,b); // expected-error {{expected _Ptr or * type}}
}
...
extern array_ptr<int> h4(void)  _Count(3) {
...
extern void f6() {
  ...
  array_ptr<int> t20  : bounds(int_array_ptr_lb, char_array_ptr_ub) = // expected-error {{pointer type mismatch}}
                    _Assume_bounds_cast_M(array_ptr<int>, i, bounds(int_array_ptr_lb, char_array_ptr_ub)); // expected-error {{pointer type mismatch}}

  array_ptr<int> t21  : bounds(int_ptr_lb, char_array_ptr_ub) = // expected-error {{pointer type mismatch}}
        _Assume_bounds_cast_M(array_ptr<int> ,i, bounds(int_ptr_lb, int_array_ptr_ub));
}

extern void f7(void *p) {
  array_ptr<int> r  _Count(3) = 0;
  ptr<int>q = 0;
  r = _Assume_bounds_cast_M(array_ptr<int> , p, count(3));
...
extern void f8() {
  ...
  t = _Assume_bounds_cast_M_N(ptr<int* _Single> , q);
  t = _Dynamic_bounds_cast_M_N(ptr<int* _Single> ,q);
  r = _Dynamic_bounds_cast_M_N(int* _Single ,q);
  p = _Assume_bounds_cast_M_N(int *,q);
...
extern void f9() {
  ...
  r = _Assume_bounds_cast_M(array_ptr<int>, h4(), bounds(r, r + 4) rel_align(int)); // expected-error {{inferred bounds for 'r' are unknown after assignment}}

  // The declared bounds of h4() do not use the value of r, so the bounds of the
  // cast expression are not invalidated.
  r = _Assume_bounds_cast_M(array_ptr<int>, h4(), count(4));
...
extern void f10() {
  int *p;
  array_ptr<int> q  _Count(3) = 0;
  int* _Single r = 0;
  int* *_Single s = 0;
 ptr<int*_Single> t = 0;
  array_ptr<int> qq  _Count(4) = 0;
  ...
  t = _Assume_bounds_cast_M(ptr<int* _Single>, q, bounds(q, q + 4) rel_align_value(sizeof(int))); // expected-error {{expected _Array_ptr type}}

  qq = _Assume_bounds_cast_M(array_ptr<int>, q, bounds(q, q + 4)  rel_align(int)) ;
  p = _Assume_bounds_cast_M(int *, q, bounds(q, q + 4) rel_align_value(sizeof(int))); // expected-error {{expected _Array_ptr type}}
}
...
extern void f11() {
  ...
  r = _Dynamic_bounds_cast_M_N(array_ptr<int>, r); // expected-error{{expected _Ptr or * type}}
  r = _Dynamic_bounds_cast_M_N(array_ptr<int>, q); // expected-error {{expected _Ptr or * type}}
  r = _Dynamic_bounds_cast_M_N(array_ptr<int>, i); // expected-error {{expected _Ptr or * type}}
  r = _Dynamic_bounds_cast_M_N(array_ptr<int>, i); // expected-error {{expected _Ptr or * type}}
  r = _Dynamic_bounds_cast_M_N(array_ptr<int>, p); // expected-error {{expected _Ptr or * type}}
  rr = _Dynamic_bounds_cast_M_N(array_ptr<int>, p); // expected-error {{expected _Ptr or * type}}
  rrr = _Dynamic_bounds_cast_M_N(array_ptr<int>, p); // expected-error {{expected _Ptr or * type}}
  r = _Dynamic_bounds_cast_M_N(array_ptr<int>, p); // expected-error {{expected _Ptr or * type}}
