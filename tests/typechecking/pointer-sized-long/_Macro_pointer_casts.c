...
extern void f3();
extern void f3(long p  _Bounds((char *)p, (char *)p + len), int len);
...
// short int g20  _Byte_count(5 * sizeof(int)) = (short int) s1;
long g21  _Byte_count(5 * sizeof(int)) = (long)s1;
long int g22  _Byte_count(5 * sizeof(int)) = (long int)s1;
unsigned long int g23  _Byte_count(5 * sizeof(int)) = (unsigned long int) s1;
// TODO: Enum size is implementation-defined
// enum E1 g24  _Byte_count(8) = EnumVal1;

// bounds
long g25  _Bounds(s1, s1 + 5) = (long)s1;
long int g26  _Bounds(s1, s1 + 5) = (long)s1;
unsigned long int g27  _Bounds(s1, s1 + 5) = (long)s1;
// TODO: Enum size is implementation-defined
// enum E1 g28  _Bounds(s1, s1 + 5) = (int)s1;

// Check that casts between pointers and long ints do not affect
// equivalence of bounds

extern int f10(int a, int* _Array b, char* _Array p  _Bounds(b, b + a));
extern int f10(int a, int* _Array b,
                char* _Array p  _Bounds(b, (int* _Array) (long) b + a));
extern int f10(int a, int* _Array b,
               char* _Array p  _Bounds(b, (int* _Array) (short) b + a)); // expected-error {{function redeclaration has conflicting parameter bounds}} \
