...
void f11(void) {
    ptr<int> x = malloc _TyArgs(int) (sizeof(int));
    free<int>(x);
}
...
void f13(void) unchecked {
    ...
    ptr<int> y = realloc _TyArgs(int) (x, 2 * sizeof(int));
    free(y);
}
...
void f21(void) {
    array_ptr<int> x  _Count(4) = malloc _TyArgs(int) (4 * sizeof(int));
    free _TyArgs(int) (x);
}

// Test you can always `free` a `calloc`d array_ptr
void f22(void) {
    array_ptr<int> x  _Count(4) = calloc _TyArgs(int) (4, sizeof(int));
    free _TyArgs(int) (x);
}
...
void f23(void) {
    array_ptr<int> x  _Count(4) = malloc _TyArgs(int) (4 * sizeof(int));
    array_ptr<int> y  _Count(8) = realloc _TyArgs(int) (x, 8 * sizeof(int));
    free _TyArgs(int) (y);
}

// Test you can always `free` a `aligned_alloc`d array_ptr
void f24(void) {
    array_ptr<int> x  _Count(4) = aligned_alloc(_Alignof(int), 4 * sizeof(int));
    free _TyArgs(int) (x);
}
