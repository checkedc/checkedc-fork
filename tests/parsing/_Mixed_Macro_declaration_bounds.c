...
extern int* _Array b  : count(3 + 2);
extern int cLen;
extern int* _Array c  : count(cLen);
extern int* _Array d  : byte_count(20);
extern int* _Array e  : byte_count(5 * sizeof(int));
extern int* _Array f  : byte_count(cLen * sizeof(int));
...
static float* _Array l  : count(5) = vals;
static _Thread_local int* _Array m  : count(5);

int* _Array b  : count(3 + 2) = 0;
...
extern void f1(int* _Array arr  : count(5)) {
  int* _Array p  : count(5) = arr;
  int* _Array q  : count(3 + 2) = arr;
  int len = 5;
  int* _Array r  : count(len) = arr;
  int* _Array s  : byte_count(20) = arr;
  int* _Array t  : byte_count(5 * sizeof(int)) = arr;
  int* _Array u  : byte_count(len * sizeof(int)) = arr;
  ...
  static int* _Array cache1  : count(5);
  static int* _Array cache1_ptr  _Bounds(cache1 - 2, cache1 + 3);
}

// Multiple declarators in one declaration
extern void f2(int* _Array arr  : count(5)) {
  int* _Array p  : count(5) = 0, * _Array q  : count(3 + 2) = arr;
  int len = 5;
  int* _Array r  : count(len) = arr, * _Array s  : byte_count(20) = arr;
  int* _Array t  : byte_count(5 * sizeof(int)) = arr,
...
extern void f3(int* _Array arr  : count(5)) {
...
// Contextual keywords
extern void f4(int* _Array arr  : count(len), int len) {
  int count = len;
  int* _Array p  : count(count) = arr;
  int unknown = 0;
...
extern void f6(int *arr checked[]  : count(5)) {
  ...
  func_arr7 arr7  checked[5]  : count(5)  = {0};
 ...
 typedef int (*_Single func_x)(int len, int* _Array : count(len));
...
extern void f7(int* _Array arr  : count(5)) {
  ...
  int* _Array t : 6 + 6, * _Array u  : count(5) = arr; // expected-error {{expected bounds expression}}
  ...
  typedef int (*_Single func_xx)(int len, int* _Array : count(badvar)); // expected-error {{undeclared identifier 'badvar'}}
  func_xx *_Array x
     _Bounds(x, x + 5) = 0;
}
