...
void failing_test_5(char* _Array pc  _Count(len), unsigned len);
void failing_test_6(char* _Array pc  _Count(len), unsigned len);
void failing_test_7(char* _Array pc  _Count(len), unsigned len);
void failing_test_8(unsigned len);

// Handle an out-of- _Bounds reference by immediately exiting. This causes
...
int main(int argc, char**_Array argv  _Count(argc)) {
  ...
  // Set up the handler for a failing  _Bounds check.  Currently the Checked C
  // clang implementation raises a SIGILL when a  _Bounds check fails.  This
  ...
  if (strcmp(argv[1], "pass1") == 0) {
    ...
    // CHECK-FAIL-6: Successful conversion to void* _Single
    ...
    // CHECK-FAIL-8: Successful conversion to const char* _Nt_array
...
// dynamic_check("abcdef", _Count(2)) -> OK.
void passing_test_1(void) {
  int* _Single q = 0;
  ...
  q = _Dynamic_bounds_cast_M(int*_Array, r+3, _Count(3));
  printf("Printable2\n");
...
void passing_test_2(void) {
  ...
  // constant folded
  q = _Dynamic_bounds_cast_M_N(int*, NULL);
  printf("Printable0\n");
  ...
  s = NULL;
  q = _Dynamic_bounds_cast_M_N(int* _Single, s);
  printf("Printable2\n");
...
void passing_test_3(void) {
  ...
  printf("Passed p1");
  void* _Single p2 = _Dynamic_bounds_cast_M_N(void* _Single, pv);
  printf("Passed p2");
  unchecked_pv = _Dynamic_bounds_cast_M_N(void *, pi);
  printf("Passed unchecked_pv");
  void* _Single p3 = _Assume_bounds_cast_M_N(void* _Single, unchecked_pv);
  printf("Passed p3");
...
void failing_test_3(void) {
  ...
  q = _Dynamic_bounds_cast_M(int* _Array, r, _Count(5));
  printf("Printable1\n");
...
// TEst dynamic checks involving possibly failig conversions to int* _Single.
void failing_test_5(char* _Array pc  _Count(len), unsigned len) {
...
// Test dynamic checks involving possibly failing conversions to void* _Single.
void failing_test_6(char* _Array pc  _Count(len), unsigned len) {
  void* _Single pv = _Dynamic_bounds_cast_M_N(void* _Single, pc);
  if (len == 0)
    puts("Unexpected Success");
  else if (pv != 0)
    puts("Successful conversion to void* _Single");
}

// Test dynamic checks involving possibly failing conversions to void *.
void failing_test_7(char* _Array pc  _Count(len), unsigned len) {
 void *pv = _Dynamic_bounds_cast_M_N(void *, pc);
  if (len == 0)
    puts("Unexpected Success");
  else if (pv != 0)
...
void failing_test_8(unsigned len) {
 const char* _Nt_array p  _Count(len) =
  ...
  if (len > 6)
    ...
    puts("Successful conversion to const char* _Nt_array");
}
